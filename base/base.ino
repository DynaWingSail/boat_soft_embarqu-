...
void setup()
{
  ...
  pinMode(led_port_side, OUTPUT);    // Make LED port writeable
  pinMode(led_star_board, OUTPUT);
  
  digitalWrite(led_port_side, HIGH); // Blink LED. 
  digitalWrite(led_star_board, HIGH);  
  delay(1);                   
  digitalWrite(led_port_side, LOW);
  digitalWrite(led_star_board, LOW);  
  delay(1); 
...
void send_data(unsigned char* data){
    ...
    { 
      ...
      //------------------Envoi des DATA au Bateau------------------//
        tx_buf[1]=consigne_voile;
        tx_buf[2]=consigne_safran;
      Serial.println();        
      NRF_Send(tx_buf);
    }
    while(!NRF_CheckAck());
  ...
  case 0 :  //par defaut commande vocala
{
  ...
  if (res==2) { // boat go port side
    digitalWrite(led_port_side, HIGH);
    digitalWrite(led_star_board, LOW); 
    consigne_safran=46;
